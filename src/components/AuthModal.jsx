      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-xl ring-1 ring-black/5">
        <div className="px-6 pt-5">
          <div className="flex gap-1 p-1 bg-gray-100 rounded-lg w-full">
            <button onClick={() => setTab('login')} className={`flex-1 py-2 rounded-md text-sm font-semibold ${tab==='login' ? 'bg-white shadow' : 'text-gray-600'}`}>Log In</button>
            <button onClick={() => setTab('register')} className={`flex-1 py-2 rounded-md text-sm font-semibold ${tab==='register' ? 'bg-white shadow' : 'text-gray-600'}`}>Create Account</button>
          </div>
        </div>
        <form onSubmit={submit} className="px-6 pb-6 pt-4 space-y-4">
          ...
          <p className="text-xs text-gray-500 text-center">This is a demo UI. No data is stored.</p>
        </form>
      </div>
    </div>
  )
